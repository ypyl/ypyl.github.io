...
| File | Purpose | Public? |
|------|---------|---------|
| `model.ts` | Types, schemas, invariants, hooks (useQuery, useMutation). Owns React Query. Owns telemetry for its operations. Owns Zustand stores — only when state outlives the component or is consumed outside it (rare; default to local `useState`). | Yes — exported in barrel |
